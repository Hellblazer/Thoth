...
public class PerceptronSimulation extends Applet implements KeyListener,
	...
	public void init() {
		...
		System.err
				.println("Thoth demo (Java applet version) Hal Hildebrand (c) 2008");
		setSize(dim_x, dim_y);
		...
		for (i = 0; i < node_size; i++) {
			Point3i pt = new Point3i(random.nextInt(dim_x),
					random.nextInt(dim_y), 0);
			UUID id = new UUID(idGen.nextLong(), idGen.nextLong());
			entities[i] = new DisplayedEntity(random, thinkTime, flipStep,
					maxVelocity, dim_x, dim_y);
			perceptrons[i] = new Perceptron(entities[i], id, pt, default_aoi,
					10, true);
	...
	public void mousePressed(MouseEvent e) {
		...
		for (int i = 0; i < node_size; i++) {
			...
			Point3i pt_clicked = new Point3i(e.getX() + originX, e.getY()
					+ originY, 0);
	...
	private void internalPaint(Graphics gobj) {
		...
		for (DisplayedEntity entity : entities) {
			entity.display(nodeNames, entity.equals(selectedEntity),
					show_edges, show_aoi, originX, originY, buff_ig);
		}
		try {
			controller.step();
		} catch (Throwable e) {
		...
		buff_ig.setPaint(fg);
		String s = " origin: (" + originX + ", " + originY + ") step: " + steps
				+ " selected: [" + nodeNames.get(selectedEntity.perceptron)
				+ "] (" + currentX + ", " + currentY + ")";
